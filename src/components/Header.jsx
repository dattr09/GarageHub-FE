...
export default function Header() {
  ...
  useEffect(() => {
    const checkLoginStatus = async () => {
      try {
        const response = await api.get("/users/me"); // Gửi yêu cầu xác thực token
  ...
  const handleLogout = async () => {
    try {
      await api.post("/users/logout"); // Gửi yêu cầu đăng xuất đến Backend
      ...
      <nav className="fixed top-0 left-0 w-full bg-gradient-to-r from-blue-300 via-blue-200 to-blue-100 shadow-md z-50 px-6 md:px-10 py-3">
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="hidden md:flex gap-6 text-gray-700 font-medium">
            ...
            <Link to="/brands" className="hover:text-blue-600 transition">
              Hãng xe
            </Link>
            <Link to="/parts" className="hover:text-blue-600 transition">
              Phụ tùng
            </Link>
            <Link to="/motos" className="hover:text-blue-600 transition">
              Xe máy
            </Link>
            <Link
              to="/repair-orders"
          ...
          <div className="flex items-center gap-4">
            ...
            {!user ? (
              ...
              <div className="relative" ref={dropdownRef}>
                <button
                  onClick={() =>
                    setOpen(open === "customer" ? false : "customer")
                  }
                ...
                >
                  ...
                  <span>
                    Chào, {user?.fullName || user?.name || "customer"}!
                  </span>
