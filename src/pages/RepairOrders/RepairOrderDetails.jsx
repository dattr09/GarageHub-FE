...
export default function RepairOrderDetails() {
    ...
    if (!order)
        return <div className="max-w-xl mx-auto mt-8 text-center text-red-500">Không tìm thấy phiếu sửa chữa.</div>;
    ...
    return (
        <div className="max-w-4xl mx-auto mt-10 bg-white rounded-2xl shadow-xl border border-gray-100 p-8">
            ...
            <div className="flex flex-col items-center gap-3 mb-8">
                ...
                <h2 className="text-3xl font-bold text-gray-800 tracking-tight">Chi tiết phiếu sửa chữa</h2>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-5">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-gray-900">{order.customerId?.fullName || order.customerId?.email}</span>
                    </div>
                    <div className="flex items-center gap-3">
                        <UserCog className="w-6 h-6 text-orange-500" />
                        <span className="font-semibold text-gray-700">Nhân viên:</span>
                        <span className="text-gray-900">{order.employeeId?.fullName || order.employeeId?.email}</span>
                    </div>
                    <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <span className={`font-bold ${order.status === "Completed" ? "text-green-700" : order.status === "Cancelled" ? "text-red-600" : "text-yellow-600"}`}>
                ...
                <div className="space-y-5">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <span className="font-semibold text-gray-700">Chi phí sửa chữa:</span>
                        <span className="text-gray-900">{order.repairCosts?.toLocaleString()}₫</span>
                    </div>
                    <div className="flex items-center gap-3">
                        <Wrench className="w-6 h-6 text-orange-600" />
                        <span className="font-semibold text-gray-700">Tổng tiền:</span>
                        <span className="text-blue-700 font-bold text-lg">{order.totalAmount?.toLocaleString()}₫</span>
                    </div>
                    <div className="flex items-center gap-3">
                        <StickyNote className="w-6 h-6 text-gray-400" />
                        <span className="font-semibold text-gray-700">Ghi chú:</span>
                        <span className="text-gray-900">{order.notes || <span className="italic text-gray-400">Không có</span>}</span>
                    </div>
                    <div className="flex items-center gap-3">
                        <FileText className="w-6 h-6 text-gray-500" />
                        <span className="font-semibold text-gray-700">Ngày cập nhật:</span>
                        <span className="text-gray-900">{order.updatedAt ? new Date(order.updatedAt).toLocaleString() : "—"}</span>
                    </div>
                </div>
            </div>
            ...
            <div className="mt-10">
                <div className="flex flex-col items-center mb-2">
                    <div className="flex items-center gap-2">
                        <Wrench className="w-6 h-6 text-orange-600" />
                        <span className="font-semibold text-blue-700 text-lg">Danh sách phụ tùng</span>
                    </div>
                </div>
                        ...
                        <tbody>
                            {order.items.map((item, idx) => (
                                <tr key={idx} className="text-center">
                                    ...
                                    <td className="px-3 py-2 border">{item.name || item.partId?.name || "--"}</td>
                                    <td className="px-3 py-2 border">{item.partId?.brandId?.name || "--"}</td>
                                    <td className="px-3 py-2 border">{item.quantity}</td>
                                    <td className="px-3 py-2 border">{item.price?.toLocaleString()}₫</td>
