import React, { useEffect, useState } from "react";
import { getAllMotos, deleteMoto } from "../../services/MotoApi";
import { useNavigate } from "react-router-dom";
import Swal from "sweetalert2";
import { Eye, Edit, Trash2, PlusCircle, ClipboardList, Search } from "lucide-react";

const MotoList = () => {
    const [motos, setMotos] = useState([]);
    const [filteredMotos, setFilteredMotos] = useState([]);
    const [searchTerm, setSearchTerm] = useState("");
    const [loading, setLoading] = useState(false);
    const navigate = useNavigate();

    useEffect(() => {
        fetchMotos();
    }, []);

    const fetchMotos = async () => {
        setLoading(true);
        try {
            const response = await getAllMotos();
            setMotos(response || []);
            setFilteredMotos(response || []);
        } catch (error) {
            console.error("Lỗi khi lấy danh sách xe máy:", error);
        } finally {
            setLoading(false);
        }
    };

    useEffect(() => {
        // Lọc danh sách xe máy theo biển số và tên người dùng
        const filtered = motos.filter(
            (moto) =>
                moto.licensePlate.toLowerCase().includes(searchTerm.toLowerCase()) ||
                (moto.userId?.fullName || "").toLowerCase().includes(searchTerm.toLowerCase())
        );
        setFilteredMotos(filtered);
    }, [searchTerm, motos]);

    const handleDelete = async (licensePlate) => {
        Swal.fire({
            title: "Xác nhận xóa",
            text: "Bạn có chắc muốn xóa xe máy này?",
            icon: "warning",
            showCancelButton: true,
            confirmButtonText: "Xóa",
            cancelButtonText: "Hủy",
        }).then(async (result) => {
            if (result.isConfirmed) {
                try {
                    await deleteMoto(licensePlate);
                    Swal.fire("Đã xóa!", "Xe máy đã được xóa thành công.", "success");
                    fetchMotos();
                } catch (error) {
                    console.error("Lỗi khi xóa xe máy:", error);
                    Swal.fire("Lỗi!", "Xóa xe máy thất bại.", "error");
                }
            }
        });
    };

    return (
        <div className="p-6 bg-gradient-to-br from-gray-50 to-gray-100 rounded-lg shadow-md max-w-6xl mx-auto mt-6">
            {/* Tiêu đề */}
            <div className="flex flex-col items-center mb-6">
                <div className="flex items-center gap-2 text-blue-600">
                    <ClipboardList className="w-8 h-8" />
                    <h2 className="text-3xl font-bold text-blue-800">Danh sách xe máy</h2>
                </div>
            </div>

            {/* Nút thêm xe */}
            <div className="flex justify-end items-center mb-4">
                <button
                    className="flex items-center gap-2 bg-green-500 text-white px-4 py-2 rounded-lg shadow hover:bg-green-600 transition"
                    onClick={() => navigate("/motos/add")}
                >
                    <PlusCircle className="w-5 h-5" />
                    Thêm xe
                </button>
            </div>

            {/* Thanh tìm kiếm */}
            <div className="relative w-full mb-6">
                <input
                    type="text"
                    placeholder="Tìm kiếm theo biển số hoặc tên người dùng..."
                    value={searchTerm}
                    onChange={(e) => setSearchTerm(e.target.value)}
                    className="w-full pl-10 pr-4 py-2 rounded-lg border-2 border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-400"
                />
                <Search className="absolute left-3 top-2.5 w-5 h-5 text-gray-400" />
            </div>

            {/* Bảng danh sách */}
            {loading ? (
                <div className="flex justify-center items-center py-10">
                    <div className="text-gray-500 text-lg">Đang tải dữ liệu...</div>
                </div>
            ) : (
                <table className="w-full border-collapse bg-white rounded-lg overflow-hidden shadow">
                    <thead>
                        <tr className="bg-gray-200 text-gray-700 text-left">
                            <th className="px-4 py-3">STT</th>
                            <th className="px-4 py-3">Biển số</th>
                            <th className="px-4 py-3">Mẫu xe</th>
                            <th className="px-4 py-3">Màu sắc</th>
                            <th className="px-4 py-3">Thương hiệu</th>
                            <th className="px-4 py-3">Người dùng</th>
                            <th className="px-4 py-3 text-center">Hành động</th>
                        </tr>
                    </thead>
                    <tbody>
                        {filteredMotos.length > 0 ? (
                            filteredMotos.map((moto, index) => (
                                <tr
                                    key={moto._id}
                                    className="hover:bg-gray-100 transition border-b last:border-none"
                                >
                                    <td className="px-4 py-3">{index + 1}</td>
                                    <td className="px-4 py-3">{moto.licensePlate}</td>
                                    <td className="px-4 py-3">{moto.model}</td>
                                    <td className="px-4 py-3">{moto.color || "Không xác định"}</td>
                                    <td className="px-4 py-3">{moto.brandId?.name || "Không xác định"}</td>
                                    <td className="px-4 py-3">{moto.userId?.fullName || "Không xác định"}</td>
                                    <td className="px-4 py-3 flex justify-center gap-4">
                                        <button
                                            className="text-blue-500 hover:text-blue-700"
                                            onClick={() => navigate(`/motos/${moto._id}`)}
                                        >
                                            <Eye className="w-5 h-5" />
                                        </button>
                                        <button
                                            className="text-yellow-500 hover:text-yellow-700"
                                            onClick={() => navigate(`/motos/edit/${moto._id}`)}
                                        >
                                            <Edit className="w-5 h-5" />
                                        </button>
                                        <button
                                            className="text-red-500 hover:text-red-700"
                                            onClick={() => handleDelete(moto.licensePlate)}
                                        >
                                            <Trash2 className="w-5 h-5" />
                                        </button>
                                    </td>
                                </tr>
                            ))
                        ) : (
                            <tr>
                                <td colSpan={7} className="text-center py-6 text-gray-500">
                                    Không tìm thấy xe nào
                                </td>
                            </tr>
                        )}
                    </tbody>
                </table>
            )}
        </div>
    );
};

export default MotoList;